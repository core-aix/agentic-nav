...
class PaperKnowledgeGraph:
    ...
    def __init__(
        ...
        self.embedding_size = embedding_gen_batch_size
    ...
    def _test_embedding_connection(self):
        ...
        try:
            ...
            print(f"Please ensure ollama is running and the model '{self.embedding_model}' is available")
    ...
    def load_papers_from_json(self, json_file_path: str, paper_dict_key: str = "results"):
        ...
        if self.limit_num_papers is not None and self.limit_num_papers > 0:
            print(f"WARNING: Number of papers limited to {self.limit_num_papers} items. Set to 'None' for all papers")
            self.papers_data = self.papers_data[:self.limit_num_papers]

        print(f"Loaded {len(self.papers_data)} papers from {json_file_path}")
    ...
    def build_graph(self):
        ...
        print(f"\nGenerating embeddings with batch size {self.embedding_size} parallel workers...")
        ...
        for info, embedding in tqdm(zip(paper_info, embeddings),
        ...
        print(f"\n✅ Built graph with {self.graph.number_of_nodes()} nodes and {self.graph.number_of_edges()} edges")
        print(f"   Papers: {len([n for n, d in self.graph.nodes(data=True) if d.get('node_type') == 'paper'])}")
        print(f"   Topics: {len([n for n, d in self.graph.nodes(data=True) if d.get('node_type') == 'topic'])}")
    ...
    def connect_similar_papers(self, similarity_threshold: float = 0.7):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(pairs), desc="Computing similarities"):
        ...
        print(f"✅ Added {connections_added} similarity edges with threshold {similarity_threshold}")
...
def main(
    ...
    kg.connect_similar_papers(similarity_threshold=0.8)

    # Save the graph to disk
    save_graph(
        graph=kg.graph,
        output_path=f"{PROJECT_ROOT}/graphs/knowledge_graph.pkl"
    )
