...
(community edition). 
**It will also populate the database with all accepted papers of the NeurIPS 2025 machine learning conference.** 
...
```
**Important note:** Generating the full graph over 6k papers can take more than 1 hour. You can find a set of pre-generated 
knowledge graphs here (the "thresh" in the file name indicates the `similarity-threshold`): [LRZ Sync+Share](https://syncandshare.lrz.de/getlink/fiFMhMLLH7FaQ3Jipqqsye/)
...
**Note:** Depending on what your graph looks like this can also take a while (> 20min for 6K papers).
...
The agent can also be used via a versatile CLI that supports LLMs beyond Ollama. 
